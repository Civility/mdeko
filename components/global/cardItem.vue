<template>
	<div class="relative flex flex-col w-full mx-auto">
		<div
			class="relative rounded overflow-hidden bg-gradient-to-r even:from-main-dark/70 from-main-light/50 odd:to-main-light/50 70 p-8 group shadow-lg flex flex-col"
		>
			<div class="relative w-32 mx-auto p-4 flex justify-center overflow-hidden rounded">
				<img class="w-full" src="/img/item.webp" :alt="data.img.alt" />
			</div>
			<h3 class="py-6 mb-0 text-dark uppercase title-line after:bg-dark before:bg-dark text-center" v-text="data.title" />
			<div class="text-center">
				<Btn svg glue link="#" class="border-dark group-hover:!border-white !w-max">
					<Svg svg="cart" class="mr-2 text-dark group-hover:!text-white" />
					<span class="text-dark group-hover:text-white">В корзину</span>
				</Btn>
			</div>
		</div>
	</div>
</template>

<script setup>
const props = defineProps({
	data: Object,
})

// const href = shallowRef()
const isHref = computed(() => `kategorii/${props.data.category}/${props.data.url}`)
</script>
